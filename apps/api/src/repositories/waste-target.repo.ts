...
export interface WasteTarget {
  ...
  wasteType: string;
  targetType: 'reduction' | 'recycling' | 'diversion' | 'intensity' | 'cost_reduction' | 'other';
...
function mapWasteTarget(row: any): WasteTarget {
  return {
    id: row.id,
...
export interface WasteTargetFilter {
  facilityId?: string;
  departmentId?: string;
  wasteType?: string;
  targetType?: string;
...
export const wasteTargetRepo = {
  async create(input: {
    ...
    const { rows } = await query<WasteTarget>(
      `INSERT INTO waste_targets (organization_id, facility_id, department_id, name, description, waste_type, target_type, baseline_value, target_value, current_value, unit, baseline_year, target_year, progress_pct, status, owner_id, notes)
    ...
    return mapWasteTarget(rows[0]);
  },

  async findById(id: string, orgId: string): Promise<WasteTarget | null> {
    const { rows } = await query<WasteTarget>(
      `SELECT * FROM waste_targets WHERE id = $1 AND organization_id = $2 AND is_deleted = FALSE`,
      [id, orgId],
    );
    return rows[0] ? mapWasteTarget(rows[0]) : null;
  },

  async listByOrganization(orgId: string, filter: WasteTargetFilter = {}): Promise<WasteTarget[]> {
    ...
    const { rows } = await query<WasteTarget>(
      `SELECT * FROM waste_targets WHERE ${where.join(' AND ')} ORDER BY created_at DESC`,
      params,
    );
    return rows.map(mapWasteTarget);
  },

  async update(id: string, orgId: string, patch: Partial<Pick<WasteTarget, 'name' | 'description' | 'wasteType' | 'targetType' | 'baselineValue' | 'targetValue' | 'currentValue' | 'unit' | 'baselineYear' | 'targetYear' | 'progressPct' | 'status' | 'ownerId' | 'notes'>>): Promise<WasteTarget | null> {
    const sets: string[] = [];
    ...
    const { rows } = await query<WasteTarget>(
      `UPDATE waste_targets SET ${sets.join(', ')} WHERE id = $${i} AND organization_id = $${i + 1} AND is_deleted = FALSE RETURNING *`,
      params,
    );
    return rows[0] ? mapWasteTarget(rows[0]) : null;
  },
  ...
  async softDelete(id: string, orgId: string): Promise<boolean> {
    const { rowCount } = await query(
      `UPDATE waste_targets SET is_deleted = TRUE, updated_at = now() WHERE id = $1 AND organization_id = $2 AND is_deleted = FALSE`,
      [id, orgId],
    );
    return (rowCount ?? 0) > 0;
  },
