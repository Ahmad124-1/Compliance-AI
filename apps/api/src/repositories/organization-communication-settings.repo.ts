...
export const mapOrgCommSettings = (row) => ({
  id: row.id,
...
export const organizationCommunicationSettingsRepo = {
  async create(input) {
    ...
    return mapOrgCommSettings(rows[0]);
  },
  ...
  async findByOrganization(orgId) {
    ...
    return rows[0] ? mapOrgCommSettings(rows[0]) : null;
  },
  ...
  async update(orgId, patch: any) {
    ...
    if (!sets.length) return this.findByOrganization(orgId);
