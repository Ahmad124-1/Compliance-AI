...
export default function PolicyAutomationPage() {
  ...
  const items = useMemo(() => (actionsData?.actions ?? []).filter((a: any) => a.actionType === 'policy_update'), [actionsData]);
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-2xl font-semibold">Policy Automation</h1>
          <p className="text-sm text-[rgb(var(--muted))]">Automate policy updates, version comparison, and distribution tracking.</p>
        </div>
      </div>
      ...
      <section className="rounded-lg border border-[rgb(var(--border-color))]">
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              {items.length === 0 ? (
                <tr><td colSpan={5} className="px-4 py-6 text-center text-xs text-[rgb(var(--muted))]">No policy automation actions available.</td></tr>
              ) : (
                items.map((item: any) => (
