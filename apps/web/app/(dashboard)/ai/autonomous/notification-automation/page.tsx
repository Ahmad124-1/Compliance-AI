...
export default function NotificationAutomationPage() {
  ...
  const items = useMemo(() => (actionsData?.actions ?? []).filter((a: any) => a.actionType === 'notification_send'), [actionsData]);
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-2xl font-semibold">Notification Automation</h1>
          <p className="text-sm text-[rgb(var(--muted))]">Automate email, SMS, WhatsApp, and escalation message delivery.</p>
        </div>
      </div>
      ...
      <section className="rounded-lg border border-[rgb(var(--border-color))]">
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              {items.length === 0 ? (
                <tr><td colSpan={5} className="px-4 py-6 text-center text-xs text-[rgb(var(--muted))]">No notification automation actions available.</td></tr>
              ) : (
                items.map((item: any) => (
