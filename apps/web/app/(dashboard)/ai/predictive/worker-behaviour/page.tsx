...
export default function WorkerBehaviourPage() {
  const { data: stats } = usePredictionStats();
  const { data: predictions } = usePredictions({ type: 'worker_intelligence' });

  const workerPredictions = useMemo(() => (predictions ?? []) as PredictionRecord[], [predictions]);

  return (
    <div className="mx-auto max-w-6xl space-y-6">
      <div>
        <h1 className="text-2xl font-semibold">Worker Behaviour</h1>
        <p className="text-sm text-[rgb(var(--muted))]">Predict grievance increases, complaint themes, department hotspots, seasonal trends and satisfaction.</p>
      </div>
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {workerPredictions.length === 0 ? (
                <tr><td colSpan={7} className="px-4 py-6 text-center text-xs text-[rgb(var(--muted))]">No worker behaviour predictions available.</td></tr>
              ) : (
                workerPredictions.map((item) => (
