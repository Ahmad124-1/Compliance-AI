...
export type RecognitionCategory = 'peer' | 'manager' | 'milestone' | 'achievement';
export type EventType = 'training' | 'safety_meeting' | 'audit' | 'town_hall' | 'factory_announcement';
export type WellbeingCheckinType = 'daily' | 'weekly' | 'monthly' | 'custom';
export type InsightSeverity = 'info' | 'warning' | 'critical';
export type InsightType = 'morale_decline' | 'burnout_risk' | 'department_attention' | 'turnover_trend' | 'engagement_campaign' | 'survey_summary';

export interface EngagementDashboard {
  overallEngagementScore: number;
  wellbeingScore: number;
  departmentRankings: Array<{ name: string; engagementScore: number; wellbeingScore: number; participationRate: number }>;
  participationRate: number;
  surveyCompletion: number;
  recognitionActivity: number;
